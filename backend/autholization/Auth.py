...
from schemes.Scheme import Token,TokenData
...
class Autholization: 
      ...
      def get_user(self,session, username: str):
            try:
      ...
      def create_access_token(self,user_data, expires_delta: Optional[timedelta] = None):
      ...
      async def get_current_user(self,token: str = Depends(oauth2_scheme)):
            ...
            try:
                  ...
                  raise credentials_exception
            user = self.get_user(get_session(), username=token_data.username)
            if not user:
                  raise credentials_exception
            return user
