...
async def add_new_user(user:Scheme.User):
    ...
    adds = Model.User(name = name,password = password)
    Model.SessionLocal.add(adds)
    Model.SessionLocal.commit()

    return {"items":Model.SessionLocal.query(Model.User).filter(Model.User.name == name).all(),
    "condition":True}
...
async def update_user(userid:int,user:Scheme.User):
